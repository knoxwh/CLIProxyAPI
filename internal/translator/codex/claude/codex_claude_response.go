...
func ConvertCodexResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	case "response.completed", "response.incomplete":
		...
		output = append(output, finalizeCodexThinkingBlock(params)...)
		output = append(output, synthesizeCodexEmptyTextBlock(params)...)
		output = append(output, stopCodexTextBlock(params)...)
...
// synthesizeCodexEmptyTextBlock emits an open+stop text block pair for a
// thinking-only turn. Claude clients fail with "Content block not found" when
// an assistant message contains only thinking, so keep a placeholder text
// block. Skipped when a tool-call turn legitimately has no text block.
func synthesizeCodexEmptyTextBlock(params *ConvertCodexResponseToClaudeParams) []byte {
	if params.TextBlockOpen || params.HasTextDelta || params.HasEmittedToolUse || params.ThinkingBlockOpen {
		return nil
	}

	output := startCodexTextBlock(params)
	output = append(output, stopCodexTextBlock(params)...)
	return output
}
